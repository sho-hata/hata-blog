...
この記事では、私が仕事やOSSプロジェクトをセットアップする際にやっていることを紹介します。主にgit hook の設定や作業メモの作成のtipsなどです。
この記事を読まれる方にとっては今更な知識が多いですが、自分のためにもまとめておきます。

## ghqでリポジトリをcloneする
gitプロジェクトは基本的にghqでcloneします。ghqはリポジトリを特定のディレクトリ`以下にcloneしてくれるツールです。リポジトリの管理がしやすくなるのでおすすめです。
